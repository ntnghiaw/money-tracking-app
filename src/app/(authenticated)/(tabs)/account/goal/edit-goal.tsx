...
const Page = () => {
  ...
  const [updateGoal, updateGoalResult] = useUpdatePlanMutation()
  const [deleteGoal, deleteGoalResult] = useDeletePlanMutation()
  const {
    data: fetchedGoal,
    isLoading,
    isSuccess,
  ...
  useEffect(() => {
    ...
    else if (deleteGoalResult.isSuccess) { 
      router.navigate('/(authenticated)/(tabs)/account/goal' as Href)
    }
  }, [updateGoalResult, deleteGoalResult])
  ...
  const handleDeleteGoal = async () => {
   try {
     await deleteGoal({ walletId, planId: id, type: 'goal' }).unwrap()
   } catch (error) {
    ...
    <View style={styles.container}>
      ...
      <View style={{ marginBottom: bottom + 24, flexDirection: 'row', gap: 12 }}>
        <Button
          ...
          isLoading={deleteGoalResult.isLoading}
          style={{ width: '48%' }}
        />
        <Button
          type={'primary'}
          ...
          isLoading={updateGoalResult.isLoading}
          style={{ width: '48%' }}
        />
      </View>
    </View>
