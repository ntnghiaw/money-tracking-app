        ...
        options={{
          ...
          header: (props) => (
            <Header
              ...
              headerRight={() => (
                <HeaderButton
                  onPress={() => router.push('/(authenticated)/(tabs)/account/create-category')}
                  type='text'
                  text={t('settings.add')}
                />
              )}
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        {categoriesFilteredByType?.map((category) => (
          <TouchableOpacity
            ...
            onPress={() =>
              router.navigate({
                pathname: `/(authenticated)/(tabs)/account/${category._id}?icon=${category.icon}&name=${category.name}&type=${category.type}`,
              })
            }
          >
