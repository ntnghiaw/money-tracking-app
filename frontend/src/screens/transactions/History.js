...
const History = ({navigation}) => {
  ...
  const Item = ({ id, category, amount, createdAt, type, description }) => (

    <TouchableOpacity style={styles.item} onPress={() => {
        navigation.navigate('Transaction');
        dispatch(read({id, category,amount,description,createdAt, type}));
      }}>
  ...
  const renderItem = ({ item }) => (
    ...
    <Item category={item.category} amount={item.amount} createdAt={item.createdAt} type={item.type} description = {item.description} id = {item._id}/>
  );
