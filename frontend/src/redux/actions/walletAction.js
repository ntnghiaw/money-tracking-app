...
const API_URL = '192.168.43.186:5000';

// Action creators
const createWalletRequest = () => ({ type: CREATE_WALLET_REQUEST });
const createWalletSuccess = (wallet) => ({ type: CREATE_WALLET_SUCCESS, payload: wallet });
const createWalletFailure = (error) => ({ type: CREATE_WALLET_FAILURE, payload: error });

const getAllWalletsRequest = () => ({ type: GET_ALL_WALLETS_REQUEST });
const getAllWalletsSuccess = (wallets) => ({ type: GET_ALL_WALLETS_SUCCESS, payload: wallets });
const getAllWalletsFailure = (error) => ({ type: GET_ALL_WALLETS_FAILURE, payload: error });

const getDetailedWalletRequest = () => ({ type: GET_DETAILED_WALLET_REQUEST });
const getDetailedWalletSuccess = (wallet) => ({ type: GET_DETAILED_WALLET_SUCCESS, payload: wallet });
const getDetailedWalletFailure = (error) => ({ type: GET_DETAILED_WALLET_FAILURE, payload: error });

const modifyWalletRequest = () => ({ type: MODIFY_WALLET_REQUEST });
const modifyWalletSuccess = (wallet) => ({ type: MODIFY_WALLET_SUCCESS, payload: wallet });
const modifyWalletFailure = (error) => ({ type: MODIFY_WALLET_FAILURE, payload: error });

const deleteWalletRequest = () => ({ type: DELETE_WALLET_REQUEST });
const deleteWalletSuccess = () => ({ type: DELETE_WALLET_SUCCESS });
const deleteWalletFailure = (error) => ({ type: DELETE_WALLET_FAILURE, payload: error });
...
export const getAllWallets = (userId) => async (dispatch, getState) => {
    ...
    try {
        const response = await axios.get(`${API_URL}/user/getAllWallets/wallets/${userId}`, {
...
export const getDetailedWallet = (userId, walletId) => async (dispatch, getState) => {
    dispatch(getDetailedWalletRequest());
    const token = getAuthToken(getState());

    try {
        const response = await axios.get(`${API_URL}/user/wallets/${userId}/${walletId}`, {
            headers: { Authorization: `Bearer ${token}` }
        });
...
export const modifyWallet = (userId, walletId, updates) => async (dispatch, getState) => {
    dispatch(modifyWalletRequest());
    const token = getAuthToken(getState());

    try {
        const response = await axios.put(`${API_URL}/user/wallets/${userId}/${walletId}`, updates, {
            headers: { Authorization: `Bearer ${token}` }
        });
...
export const deleteWallet = (userId, walletId) => async (dispatch, getState) => {
